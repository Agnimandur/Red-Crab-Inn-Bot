...
def leaderboard(prefix,game):
  ...
  for key in db.keys():
    if key.startswith(prefix):
      #[score,user id]
      score = db[key][1] if game=='reaper' else db[key][0]+r*db[key][1]
...
def leaderboardEmbed(guild,prefix,game):
  ...
  for person in rankList:
    ...
    i += 1
  description = "Top 10 reaper leaderboard. The target to win is **{p}** points.".format(p=db["REAPER GAME "+prefix][2]) if game=='reaper' else "Top 10 bitcoin investors leaderboard."
